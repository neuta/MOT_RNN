...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None, ref_track=None):
    ...
    if ref_track is not None and not isinstance(ref_track, np.ndarray):
        ref_track = ref_track.cpu().numpy()
    ...
    if ref_boxes is not None and len(ref_boxes) > 0:
        ...
        print(ref_corners3d.shape)

        if ref_labels is None and ref_track is None:
            fig = draw_corners3d(ref_corners3d, fig=fig, color=(0, 1, 0), cls=ref_scores, max_num=100)
        
        elif ref_labels is None and len(ref_track) > 0:

            for k in range(ref_track.min(), ref_track.max() + 1):
                print("k",k)
            ...
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                ...
                mask = (ref_labels == k)       
                print("mask",mask)
                print("ref_scores[mask]",ref_scores[mask]) 
